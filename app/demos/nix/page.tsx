...
// 1. Data Structure - Edit titles, links, and handle 1 or 2 inputs
const demoData = {
  ...
  skus: [
    {
      ...
      originalLink: "https://voidcycling.com/products/core-powerstretch-cycling-jersey-women-black?pr_prod_strat=jac&pr_rec_id=396112e45&pr_rec_pid=15431621607804&pr_ref_pid=15249327522172&pr_seq=uniform",
      inputs: [
...
const SourceLinkButton = ({ href }: { href: string }) => (
  ...
  >
    Source Link
    <svg width="10" height="10" viewBox="0 0 15 15" fill="none" xmlns="http://www.w3.org/2000/svg" className="shrink-0"><path d="M3.64645 11.3536C3.45118 11.1583 3.45118 10.8417 3.64645 10.6464L10.2929 4L7.5 4C7.22386 4 7 3.77614 7 3.5C7 3.22386 7.22386 3 7.5 3L11.5 3C11.6326 3 11.7598 3.05268 11.8536 3.14645C11.9473 3.24022 12 3.36739 12 3.5L12 7.5C12 7.77614 11.7761 8 11.5 8C11.2239 8 11 7.77614 11 7.5L11 4.70711L4.35355 11.3536C4.15829 11.5488 3.84171 11.5488 3.64645 11.3536Z" fill="currentColor" fillRule="evenodd" clipRule="evenodd"></path></svg>
  </a>
);

const MetadataLabel = ({ label, value, isLink = false }: { label: string; value: string; isLink?: boolean }) => (
  <div className="text-[13px] leading-relaxed flex items-start sm:items-center gap-3">
    <span className="font-medium text-neutral-400 uppercase tracking-wider text-[10px] w-8 mt-1 sm:mt-0 shrink-0">{label}</span>
    {isLink ? (
      <a href={value} target="_blank" rel="noopener noreferrer" className="text-black hover:text-neutral-500 transition-colors inline-flex items-center gap-1 break-all sm:break-normal">
        Source Link
        <svg width="10" height="10" viewBox="0 0 15 15" fill="none" xmlns="http://www.w3.org/2000/svg" className="shrink-0"><path d="M3.64645 11.3536C3.45118 11.1583 3.45118 10.8417 3.64645 10.6464L10.2929 4L7.5 4C7.22386 4 7 3.77614 7 3.5C7 3.22386 7.22386 3 7.5 3L11.5 3C11.6326 3 11.7598 3.05268 11.8536 3.14645C11.9473 3.24022 12 3.36739 12 3.5L12 7.5C12 7.77614 11.7761 8 11.5 8C11.2239 8 11 7.77614 11 7.5L11 4.70711L4.35355 11.3536C4.15829 11.5488 3.84171 11.5488 3.64645 11.3536Z" fill="currentColor" fillRule="evenodd" clipRule="evenodd"></path></svg>
      </a>
    ) : (
    ...
    <main className="min-h-screen bg-white text-neutral-900 selection:bg-neutral-100 font-sans">
      ...
      <header className="border-b border-neutral-100">
        <div className="max-w-[1600px] mx-auto px-6 py-5 md:py-6">
          <h1 className="text-base md:text-lg font-bold tracking-widest text-black uppercase">Afterlight</h1>
        </div>
      </header>
      ...
      <div className="max-w-[1600px] mx-auto px-6 py-12 md:py-32">
        ...
        {demoData.skus.map((sku, index) => (
          <div key={sku.id} className={`${index !== 0 ? 'mt-24 pt-16 md:mt-40 md:pt-20 border-t border-neutral-100' : ''}`}>
            ...
            {/* The Pipeline Visual: Responsive layout changes flow from horizontal (PC) to vertical (Mobile) */}
            <div className="grid grid-cols-1 lg:grid-cols-[minmax(0,4fr)_auto_minmax(0,7fr)] gap-0 lg:gap-16 items-start">
              ...
              <div className="flex flex-col w-full">
                ...
                {/* Inputs: Stays compact on mobile so original data is easily scanned */}
                <div className={`grid gap-3 md:gap-4 mb-6 ${sku.inputs.length === 1 ? 'grid-cols-1 w-2/3 max-w-[300px]' : 'grid-cols-2'}`}>
                  {sku.inputs.map((src, idx) => (
                    <div key={idx} className="relative aspect-square bg-[#fafafa] rounded-md overflow-hidden border border-neutral-100">
                      <Image
                         src={src}
              ...
              <div className="hidden lg:flex h-full items-center justify-center pt-8">
                <svg width="32" height="32" viewBox="0 0 24 24" fill="none" className="text-neutral-300 stroke-current stroke-[1.5]">
                  <path d="M4 12H20M20 12L13 5M20 12L13 19" strokeLinecap="round" strokeLinejoin="round"/>
                </svg>
              </div>

              {/* MOBILE ARROW (Down) */}
              <div className="flex lg:hidden justify-center py-10 opacity-60">
                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" className="text-neutral-400 stroke-current stroke-[1.5]">
                  <path d="M12 4V20M12 20L5 13M12 20L19 13" strokeLinecap="round" strokeLinejoin="round"/>
                </svg>
              </div>
              ...
              <div className="flex flex-col w-full">
                ...
                {/* Outputs: Full width stacked on mobile for max detail, 3-column on desktop */}
